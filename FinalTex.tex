...
\begin{table}[h!]
  \caption{Exceptions in C++}
  \label{table:EX}
  \begin{tabular}{|l|p{0.5\linewidth}|}
    \toprule % toprule here
    \textbf{Exception} & \textbf{Short Description} \\
    \midrule % midrule here
    std::exception & An exception and parent clas of all standard C++ exceptions\\
    \hline
    std::bad\_alloc & This can be thrown by the New method\\
    \hline
    std::bad\_cast & This can be thrown by the dynamic\_cast method\\
    \hline
    std::bad\_exception & This is useful when handling unexpected exceptions in a C++ program\\
    \hline
    std::bad\_typeid & This can be thrown by the typeid method\\
    \hline
    std::logic\_error & An exception that can theoretically be detected by reading the code\\
    \hline
    std::domain\_error & An exception thrown when a mathematically invalid domain is used\\
    \hline
    std::invalid\_argument & An exception thrown due to invalid arguments\\
    \hline
    std::length\_error & An exception thrown when an std::string that is too large is created\\
    \hline
    std::out\_of\_range & This is can be thrown by the at method, used generally with an std::vector\\
    \hline
    std::runtime\_error & An exception that cannot theoretically be detected by reading the code\\
    \hline
    std::overflow\_error & An exception thrown when a mathematical overflow occurs\\
    \hline
    std::range\_error & An exception thrown when once tries to store a value which is out of range\\
    \hline
    std::underflow\_error & An exception thrown when a mathematical underflow occurs\\
    \hline
  \end{tabular}
\end{table}

The use of all these exceptions allows a great degree of control for the programmer to issue many methods whenever cases of exceptions do happen within their code. It is unfortunate, however, that the code blocks require a great deal of writing especially if the programmer needs multiple catches for various exceptions coupled wwith syntaxes that are not very easily readable due to the confusion between the try-catch blocks and where the throws are going to be. However, because exception handling in C++ is well documented and has been developed over the course of many iterations throughout its development and standardizations, it is assured that all exception handling is reliable, works very well for all types of implementations and applications and is a very useful tool for programmers.
...
\bibitem{Introduction to Static and Dynamic Typing}
...
\bibitem{Implicit type conversion in C++}
...
\bibitem{Explicit type conversion in C++}
...
\bibitem{Type Conversion Rules in C++}
...
\bibitem{Operator Precedence in C++}
...
\bibitem{Parameter Passing Techniques in C++}
...
\bibitem{Function Overloading in C++}
...
\bibitem{Generic Functions in C++}
...
\bibitem{Difference of Function and Subroutine}
