...
\subsubsection{Binding, Scoping, Referencing}
This section covers the C++ languages ability of binding, scoping, and referencing.
...
\textbf{Reserved Words} All programming languages have certain keywords that are made available to them in order to create the codes they need to create. Words such as \textit{if, for, while, do, continue, break, return, True, False} are only among of the few keywords that have a specific role in C++. These keywords are case-sensitive and will not be recognized by the compiler if used any differently. The downside of having so many keywords is not being able to memorize all of them and their specific uses. 

It also worthy to note that when a user-defined function is declared or defined, one cannot use the name of said function as a variable name, further improving the readability of a specific line of code in C++. Unfortunately, this does affect the writability since this does limit the freedom of the programmer to bind a specfic name. 

\textbf{Scoping} C++ has a 
...
The approach uses a single function with all the possible parameters within it. We exploit the feature of having default argument values NULL.
        ...
        if(is.integer(a)){
            ...
            if(isFALSE(b))
                ...
                if(!isFALSE(c))
                    ...
                    stop(``Variable c is not initialized'')
            }
