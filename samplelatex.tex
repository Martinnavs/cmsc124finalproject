...
logical (TRUE/FALSE)
integer (1L, 2L) -writability, if wa ka kabalo nga ingon ani ang rule mahibulong ka
real/numeric (1, 3.2) -default for numbers without L
complex (3 + 2i)
...
syntax: readability suffers
c() does not indicate any relation to vectors, which
writability suffers
...
factor - more on categorical values, vector with distinct values of the elements as labels
factor()

data frames - tabular data objects, each column can contain different data, list of vectors of equal length
data.frame()
