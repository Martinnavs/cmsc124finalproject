...
\textbf{Operators and their overloads} C++ has a great number of operators existing within the language. The table of precedence rules shows the which among the operators work first 	\subsubsection{Statements and Control Structures}
...
Similar to classes, structs are very useful in object-oriented programming. It provides another way of grouping data elements under one name. It also provides for a means to implement plain-old-data (POD) types, which are classes without constructors/destructors.
...
One major advantage of function overloading is having the same name for functions which do the same things, thereby improving readability.
...
\section{Conclusion}
<<<<<<< HEAD
After the extensive critique of both languages, one can see the advantages and disadvantages of C++ and R as programming languages.
